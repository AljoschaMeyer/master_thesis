...
\begin{itemize}
  ...
  \item For every range fingerprint $\ifp{x}{y}{X_j}$ in the message, it does one of following:
    \begin{caselist}
      ...
      \case[Recursion Anchor] \label{def-recursion-anchor} The node may add the range item set $\iisnatural{x}{y}{X_i}{0}$ to the response. If $\abs{\range{x}{y}{X_j}} \leq 1$, it must do so.
...
\case[Recursion Anchor] \label{case-ifp-anchor} If $\mathcal{X}_i$ adds the range item set $\iisnatural{x}{y}{X_i}{0}$, then \cref{case-iis} applies when the other node receives the response, with the roles reversed.
