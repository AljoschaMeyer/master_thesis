...
Recall that we assume $U$ to be finite and with the greatest element $\top$, which never occurs as the lower boundary of an interval. For $x \in U \setminus \{\top\}$ we can thus denote by $\successor{x}$ the unique $z \in U$ such that $x \prec z$ and there exists no $y \in U$ such that $x \prec y \prec z$. Upon sending or receiving an interval from $x$ to $y$ for which both nodes know that the sending node holds $x$, the receiving node adds $x$ to its set of items, and then both nodes proceed as if the interval went from $\successor{x}$ to $y$.
